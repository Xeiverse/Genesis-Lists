...
| GET | `/api/auth/oidc/start` | No | Web: `302` to the IdP authorize URL. Android (`?client=android`): `200` HTML that navigates to the IdP so Custom Tabs persist `genesis_oidc_state`. `404` when OIDC is disabled |
