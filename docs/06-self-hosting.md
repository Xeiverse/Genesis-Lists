...
Until a release image is on GHCR, `--build` is required. After `v0.3.0` is published, `docker compose pull` uses `ghcr.io/xeiverse/genesis-lists`.
...
| `GENESIS_LISTS_VERSION` | No | `0.3.0` | Compose-only. Image tag to pull, and the `APP_VERSION` passed into the container |
...
Image tags such as `0.3.0` are immutable. Pin `GENESIS_LISTS_VERSION` in `.env`. Do not follow `latest` for an install you need to roll back.

1. Back up (above).
2. Set `GENESIS_LISTS_VERSION` to the new tag (for example `0.3.0`).
