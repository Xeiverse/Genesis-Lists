...
- [x] **REQ-SHARE-01** Owner puts members via share dialog / `PUT /api/lists/{id}/members`; members see the list on `GET /api/lists` with `isOwner: false` and correct `ownerUsername` — *API test + browser*
...
- [x] **REQ-SHARE-06** `GET /api/users` returns all users’ id and username for a signed-in caller — *API test*
...
- [x] Health reports `schemaVersion: 2` — *API test*
